...
public class BuyOrderGemFireTest extends TestCase {
			...
			public void onEvent(CqEvent cqEvent) {
				...
				if (baseOperation.isCreate()) {
					...
					if (event instanceof BuyOrderPlaced) {
						buyOrderPlaced = (BuyOrderPlaced) event;
					}
				}
			}
		};
	...
	public void testFillBuyOrder() throws Exception {
		...
		try {
			...
			assertEquals(buyOrder.quote().price().multipliedBy(buyOrder.quantityOfSharesOrdered()),
					holding.totalValue());
	...
	private BuyOrder buyOrderFixture() {
		return new BuyOrder(AccountId.unique(), new Quote(new TickerSymbol(TICKER), PRICE), NUMBER_OF_SHARES,
				new Money("9.99"));
	}
}
